...
public class Adventure {
    ...
    private void onboarding() throws UnknownConnectionException {
        ...
        server.sendMessage(connectionId, "Your friend is entering the Michigan Tech Esports team and needs a computer that he can rely on to help him win the tournament.");
        server.sendMessage(connectionId, "However, both of you are poor college students and do not have the funds to pay for this gaming computer.");
        server.sendMessage(connectionId, "Instead of saving up and potentially not participating this year, you decide to steal parts from a local supplier.");
        server.sendMessage(connectionId, "You must steal the best possible parts from the supplier and not get caught in the process.");
        server.sendMessage(connectionId, "Send START to begin the game.");
    }
    ...
    public String processCommand(String data) throws UnknownConnectionException {
